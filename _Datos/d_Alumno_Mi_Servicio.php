...
//                                $conn->rollBack();
                ...
                for($i=0; $i < $renglones; $i++ ){
                    ...
                    if ($stmt4){
                        $result4=$stmt4->execute(array($clave_SS, $calendario_Entrega_RptBim[$i][0],
                            $calendario_Entrega_RptBim[$i][1],$i+1,1,1,$calendario_Entrega_RptBim[$i][2],
                                $obj_SS->get_Id_Division()));
                        if (!$result4 ===FALSE){
                                if($stmt4->rowCount() > 0){
...
//        }
    ...
    function Actualizar_Aceptacion_Doc_($id_SS, $id_Doc, $id_Version, 
            $id_Estatus, $id_administrador, $nota, $id_Usr_Destinatario, $correo_usr, $carrera_usr, $desc_documento, $id_division){
        $mensaje_Transacciones = '';
        $conn = '';
        $jsondata = array();
        $id_tema = 0;
