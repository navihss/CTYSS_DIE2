...
require_once $_SERVER["DOCUMENT_ROOT"] . '/CTYSS_DIE2/Logger.php';
...
class d_coord_jdpto_Aprobar_Propuesta
{
    ...
    function Actualizar_Aceptacion_Doc(
        ...
        $logFilePath = $_SERVER["DOCUMENT_ROOT"] . '/CTYSS_DIE2/logs/propuestas.log';
        $logger = new Logger($logFilePath);

        $logger->info("Proceso iniciado");
        $logger->info("id_propuesta_doc: " . $id_propuesta_doc);
        $logger->info("id_documento_doc: " . $id_documento_doc);
        $logger->info("id_version_doc: " . $id_version_doc);
        $logger->info("id_estatus: " . $id_estatus);
        $logger->info("id_usuario: " . $id_usuario);
        $logger->info("nota: " . $nota);
        $logger->info("fecha_registro_doc: " . $fecha_registro_doc);
        $logger->info("id_profesor: " . $id_profesor);
        $logger->info("correo_profesor: " . $correo_profesor);
        $logger->info("titulo_propuesta: " . $titulo_propuesta);
        $logger->info("desc_corta_doc: " . $desc_corta_doc);
        $logger->info("id_division: " . $id_division);
        ...
        try {
            ...
            if ($conn === false) {
                $logger->error("Error de conexión: " . $cnn->getError());
                throw new Exception($cnn->getError());
            }
